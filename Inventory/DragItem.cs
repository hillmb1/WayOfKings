    ...
    public void OnEndDrag(PointerEventData data)
    {
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            else
            {
				...
				dropItem.GetComponent<SpriteRenderer> ().sprite = GetComponent<ItemOnObject>().item.itemIcon;
                dropItem.AddComponent<PickUpItem>();
                dropItem.GetComponent<PickUpItem>().item = this.gameObject.GetComponent<ItemOnObject>().item;               
				dropItem.transform.localPosition = GameObject.FindGameObjectWithTag("Player").transform.localPosition + new Vector3(0f, 0f, 0.1f);
                inventory.OnUpdateItemList();
